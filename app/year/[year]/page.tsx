...
/* PENTING: Keluar 3 tingkat (../../../) karena file ini berada di dalam 
   folder app > year > [year]. Ini akan mengarah ke folder 'lib' di root.
*/
...
export default function YearCollectionPage({ params }: { params: { year: string } }) {
  ...
  useEffect(() => {
    async function fetchPapers() {
      setLoading(true);
      // Mengambil data dari tabel 'publikasi_ilmiah' berdasarkan tahun di URL
  ...
  const handleAdminAccess = (e: React.FormEvent) => {
    e.preventDefault();
    // Memeriksa password terhadap Environment Variable 'NEXT_PUBLIC_ADMIN_CODE'
  ...
  const handleDelete = async (id: string, title: string) => {
    if (confirm(`Hapus permanen publikasi: "${title}"?`)) {
  ...
  return (
    <div className="min-h-screen p-8 md:p-16 bg-slate-50">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Navigasi - Menggunakan Biru Prospect (#003193) */}
        ...
        {/* Form Aktivasi Mode Admin */}
        {!isAdmin && (
          <form onSubmit={handleAdminAccess} className="mb-10 p-6 bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4 items-center">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex-1">Mode Manajemen (Edit/Hapus):</p>
            <input 
              type="password" 
        ...
        <div className="grid gap-6">
          {loading ? (
            <p className="text-slate-400 italic">Memuat data riset...</p>
          ) : papers.length > 0 ? (
            papers.map((paper) => (
              <div key={paper.id} className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-50 group transition-all">
                <div className="flex flex-col md:flex-row justify-between items-start gap-8">
                  ...
                  <div className="flex flex-wrap gap-3 w-full md:w-auto">
                    {/* Tombol Baca - Warna Tema Prospect (Hijau-Biru) */}
                    <a 
                      href={paper.pdf_url} 
                      target="_blank" 
                    ...
                    {isAdmin && (
                      <div className="flex gap-2 w-full md:w-auto">
                        <button className="flex-1 md:flex-none bg-slate-100 text-slate-600 px-6 py-4 rounded-xl font-bold text-[10px] uppercase hover:bg-slate-200">Edit</button>
                        <button 
                          onClick={() => handleDelete(paper.id, paper.title)}
                          className="flex-1 md:flex-none bg-red-50 text-red-600 px-6 py-4 rounded-xl font-bold text-[10px] uppercase hover:bg-red-100"
                        >
                          Hapus
                        </button>
                      </div>
            ...
            <div className="py-20 text-center border-2 border-dashed border-slate-100 rounded-[2rem]">
              <p className="text-slate-400 italic">Tidak ada publikasi ditemukan untuk tahun ini.</p>
            </div>
          )}
        </div>
      </div>
